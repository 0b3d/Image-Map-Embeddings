...
import pandas as pd
import cv2 
from slutils.Tile import Tile

class LocationM(Tile):
    def __init__(self, dataset, ID, city, HD, base_index='local', tile_style='s2v'):
        ...
        if tile_style == 's2v':
            tileDir = os.path.join( os.environ['dev'], 'docker-cvloc', 'temp', dataset) # map Directory
        elif tile_style == 's2v_height':
            tileDir = os.path.join( self.datasetPath, 'tiles_height_' + city + '_2019') # map Directory
        else:
        ...
        Tile.__init__(self, tileDir, pano_id + '.png', yaw)
        self.pano_id = pano_id

if __name__ == "__main__":
    loc = LocationM("hudsonriver5k", 4000, 'manhattan', base_index='global')
    print(loc)
